...
function __promptline_ps1 {
  ...
  # section "x" header
  slice_prefix="${x_bg}${sep}${x_fg}${x_bg}${space}"
  slice_suffix="$space${x_sep_fg}"
  slice_joiner="${x_fg}${x_bg}${alt_sep}${space}"
  slice_empty_prefix="${x_fg}${x_bg}${space}"
  [ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "x" slices
...
function __promptline_right_prompt {
  ...
  # section "x" header
  slice_prefix="${x_sep_fg}${rsep}${x_fg}${x_bg}${space}"
  slice_suffix="$space${x_sep_fg}"
  slice_joiner="${x_fg}${x_bg}${alt_rsep}${space}"
  slice_empty_prefix=""
  # section "x" slices
  __promptline_wrapper \
    "$(__promptline_vcs_branch)" "$slice_prefix" "$slice_suffix" \
...
function __promptline {
  ...
  local c_bg="${wrap}48;5;2${end_wrap}"
